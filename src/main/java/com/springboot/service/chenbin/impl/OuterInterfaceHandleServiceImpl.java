...
import com.springboot.popj.json_data.JSONJyhtxx;
...
public class OuterInterfaceHandleServiceImpl implements OuterInterfaceHandleService {
    ...
    @Override
    public Object getContractRecordxx(Map<String,String> params) {
        ...
        if (map.get("statusCode").toString().equals("-1") && StringUtils.isNotEmpty(map.get("msg").toString())){
            throw new ZtgeoBizException(map.get("msg").toString());
        }
        ...
        jyhtxx.setContractRecordNumber(contractQueryVo.getBah());
        jyhtxx.setContractRecordTime(DateUtils.stringToDate(contractQueryVo.getBasj(),DateUtils.JAVA_YYYY_MM_DD));
        jyhtxx.setRegistrationNumber(contractQueryVo.getQyslh());
        jyhtxx.setAuditStatus(contractQueryVo.getStatus());
        return  jyhtxx;
    }
